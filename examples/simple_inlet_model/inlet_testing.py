...
from stanshock.system.geometry import (  # Or use the initialize_geometry helper function with upper and lower walls
    AsymmetricBox,
    LinearInterpolator,
)
...
regions = {
    "domain": (x_combustor_start, x_combustor_end),
    "external": (x_ramp_start, x_combustor_start),
}

xf = np.linspace(x_ramp_start, x_combustor_end, N_x + 1)
ramp_lower_wall = LinearInterpolator(
    xp=[x_ramp_start, x_combustor_start], fp=[y_ramp_start, 0]
)
combustor_lower_wall = LinearInterpolator(
    xp=[x_combustor_start, x_combustor_end], fp=[0, 0]
)
ramp_upper_wall = LinearInterpolator(
    xp=[x_cowl_start, x_combustor_start], fp=[h_const, h_const]
)
combustor_upper_wall = LinearInterpolator(
    xp=[x_combustor_start, x_combustor_end], fp=[h_const, h_const]
)

lower_wall_ramp_y_values = ramp_lower_wall(_time=0, x=xf[xf <= x_combustor_start])
lower_wall_combustor_y_values = combustor_lower_wall(
    _time=0, x=xf[(xf >= x_combustor_start) & (xf <= x_combustor_end)]
)

lower_wall_y_values = np.concatenate(
    (lower_wall_ramp_y_values, lower_wall_combustor_y_values)
)

upper_wall_ramp_y_values = ramp_upper_wall(
    _time=0, x=xf[(xf >= x_cowl_start) & (xf <= x_combustor_start)]
)
...
upper_wall_combustor_y_values = combustor_upper_wall(
    _time=0, x=xf[(xf >= x_combustor_start) & (xf <= x_combustor_end)]
)
...
geometry = AsymmetricBox(
    xf=xf,
    regions=regions,
    lower_wall=(xf, lower_wall_y_values),
    upper_wall=(xf[(xf >= x_combustor_start)], upper_wall_combustor_y_values),
)  # Geometry definition goes in here
...
for aoa in aoa_range:
    inlet.compute_combustor_inlet_properties(
        n_shocks=3, flow_deflection_angles=[18 + aoa, 18, 0]
    )
