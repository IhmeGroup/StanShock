...
mt = 3
"""Number of ghost nodes for the solver."""
...
@njit(_double_3D(_double_1D, _double_1D, _double_1D, _double_2D, _double_1D))
        ...
        for N in range(nLR):  # !left edge and right edge
            ...
            for iVar in range(nVar):
                ...
                B[0] = B1 * (CStencil[0 + NO, iVar] - 2.0 * CStencil[1 + NO, iVar] + CStencil[
                    2 + NO, iVar]) ** 2.0 + B2 * (
                               3.0 * CStencil[0 + NO, iVar] - 4.0 * CStencil[1 + NO, iVar] + CStencil[
                           2 + NO, iVar]) ** 2
                B[1] = B1 * (CStencil[-1 + NO, iVar] - 2.0 * CStencil[0 + NO, iVar] + CStencil[
                    1 + NO, iVar]) ** 2.0 + B2 * (CStencil[-1 + NO, iVar] - CStencil[1 + NO, iVar]) ** 2
                ...
                for iStencil in range(mt):  # iterate through each stencil
                    ...
                    CINT = W[N, iStencil, 0] * CStencil[0 + iStencilO, iVar] + W[N, iStencil, 1] * CStencil[
                        1 + iStencilO, iVar] + W[N, iStencil, 2] * CStencil[2 + iStencilO, iVar]
...
@njit(_double_2D(_double_2D, _double_2D, _double_2D, _double_3D, _double_1D))
def LF(rLR, uLR, pLR, YLR, gamma):
    ...
    lambdaMax = 0.0
    for iFace in range(nFaces):
        a = max(np.sqrt(gamma[iFace] * pLR[0, iFace] / rLR[0, iFace]),
...
@njit(_double_1D(_double_2D, _double_1D))
def get_R(Y, molecularWeights):
    """
...
@njit(_double_1D(_double_1D, _double_2D, _double_1D, _double_2D, _double_2D))
def get_Cp(T, Y, TTable, a, b):
    """
...
class ThermoTable:
    ...
    def __init__(self, gas):
        ...
        self.dT = 100.0
        self.TMax = 9950.0
        self.T = np.arange(self.TMin, self.TMax,
                           self.dT)  # vector of temperatures assuming thermal equilibrium between species
        nT = len(self.T)
    ...
    def get_R(self, Y):
        """
        This method computes the mixture-specific gas constant
        ...
        return get_R(Y, self.molecularWeights)
    ...
    def get_Cp(self, T, Y):
        ...
        return get_Cp(T, Y, self.T, self.a, self.b)
...
def smoothing_function(x, x_shock, delta, phi_left, phi_right):
    """
    ...
    Args:
        ...
        x_shock:  the mean of the shock location
        phi_left:  the value of the variable on the left side
        phi_right:  the value of the variable on the right side

    """

    dphidx = (phi_right - phi_left) / delta
    phi = (phi_left + phi_right) / 2.0 + dphidx * (x - x_shock)
    phi[x < (x_shock - delta / 2.0)] = phi_left
    phi[x > (x_shock + delta / 2.0)] = phi_right
    return phi


def dSFdx(x, x_shock, delta, phi_left, phi_right):
    """
    ...
    dphidx[x < (x_shock - delta / 2.0)] = 0.0
    dphidx[x > (x_shock + delta / 2.0)] = 0.0
    return dphidx
...
class StanShock:
    ...
    def add_probe(self, probeLocation, skipSteps=0, probeName=None):
        """
    ...
    def viscous_flux(self, r, u, p, Y, gamma):
        ...
        def viscous_flux_function(self, rLR, uLR, pLR, YLR):
            """
            ...
            nT = self.n + 2
            T = np.zeros(nT)
            T[:-1] = self.ThermoTable.get_temperature(rLR[0, :], pLR[0, :], YLR[0, :, :])
            T[-1] = self.ThermoTable.get_temperature(np.array([rLR[1, -1]]),
                                                     np.array([pLR[1, -1]]),
    ...
    def advance_diffusion(self, dt):
        ...
        # 1st stage of RK2
        rhs = self.viscous_flux(r, u, p, Y, gamma)
        r1 = r + dt * rhs[:, 0]
        ru1 = ru + dt * rhs[:, 1]
        E1 = E + dt * rhs[:, 2]
        rY1 = rY + dt * rhs[:, mn:]
        (r1, u1, p1, Y1) = self.conservative_to_primitive(r1, ru1, E1, rY1, gamma)
        # 2nd stage of RK2
        rhs = self.viscous_flux(r1, u1, p1, Y1, gamma)
        r = 0.5 * (r + r1 + dt * rhs[:, 0])
    ...
    def pressure_rise(t, p, peakWidth=10):
        ...
        of the first two peaks in the logarithmic derivative of the
    ...
    def advance_simulation(self, tFinal):
        """
    ...
    def optimize_driver_insert(self, tFinal, tradeoffParam=1.0,
        ...
        def midpoint_vector(x_min, x_max, nMidpoints):
            ...
            dx = (x_max - x_min) / float(nMidpoints)
            return x_min + dx / 2.0 + dx * np.arange(0, nMidpoints)
